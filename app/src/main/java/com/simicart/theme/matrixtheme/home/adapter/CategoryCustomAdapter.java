...
public class CategoryCustomAdapter extends BaseAdapter {
    ...
    private ArrayList<Theme1SpotProduct> mOrderProducts;
    private Context mContext;
    private LayoutInflater mInflater;

    public CategoryCustomAdapter(Context context,
                                 ArrayList<Theme1SpotProduct> products) {
        mContext = context;
    ...
    public View getView(int pos, View convertView, ViewGroup parentView) {
        ...
        final Theme1SpotProduct product = mOrderProducts.get(pos);
        ...
        if (product.getID().equals("fake")) {
            ...
            ArrayList<String> urlImages = product.getmImages();
    ...
    protected void viewOrderProduct(Theme1SpotProduct orderProduct) {
        CategoryDetailFragment fragment = CategoryDetailFragment
                .newInstance();
        fragment.setCategoryId(orderProduct.getID());
        fragment.setCategoryName(orderProduct.getName());
        fragment.setUrlSearch("products");
        fragment.setTypeSport(orderProduct.getType());
        if(orderProduct.getProducts() != null && orderProduct.getProducts().size() > 0){
            String ids = arrayToString(orderProduct.getProducts());
