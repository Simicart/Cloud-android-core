...
public class ShippingMethodBlock extends SimiBlock implements ShippingDelegate {
	...
	public void setShippingMethods(
		...
		if (null != list_shippingMethods) {
			...
			for (int i = 0; i < list_shippingMethods.size(); i++) {
				...
				String s_incl_tax = shppingMethod.getS_method_fee_incl_tax();

				Float price = shppingMethod.getPrice();
				Float incl_tax = null;
				try {
				...
				tv_title.setText(shppingMethod.getServiceName(),
				...
				if (shppingMethod.getDescription() != null
						&& !shppingMethod.getDescription().equals("")
						&& !shppingMethod.getDescription().equals("null")) {
					tv_content.setText(shppingMethod.getDescription());
...
//					if(list_shippingMethods.get(0).isS_method_selected() == false) {
...
//				}
	...
	private void handleViewShippingAndPayment(boolean check) {
		...
		if (check == true) {
			...
			reviewOrder.setActionArrowDown(0);
			 if (ConfigCheckout.checkPaymentMethod == true) {
				 ...
				 if(!DataLocal.isTablet){
					 ...
					 reviewOrder.setActionArrowUp(1);
				 }
...
//					reviewOrder.updateShippingMethod(shippingMethod);
			...
			try {
				param = paramToRequest(shippingMethod);
