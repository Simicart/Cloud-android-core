...
public class AddressBookDetailBlock extends SimiBlock implements
	...
	private void createPrefix() {
		et_prefix.setHint(Config.getInstance().getText("First Name") + " (*)");
...
//		et_fullname.setHint(Config.getInstance().getText("Name") + " (*)");
	}

	private void createSuffix() {
		et_suffix.setHint(Config.getInstance().getText("Last Name") + " (*)");
		String suffix = mAddressBookDetail.getSuffix();
		if (null != suffix && !suffix.equals("") && !suffix.equals("Last Name")) {
...
//		}
	...
	private void createCountry() {
		String countryname = mAddressBookDetail.getCountry().getName();
	...
	private void createState(ArrayList<CountryAllowed> listCountry) {
		ArrayList<String> states = getStateFromCountry(
				mAddressBookDetail.getCountry().getName(), listCountry);
		...
		String state = "";
		if(mAddressBookDetail.getStates() != null){
			state = mAddressBookDetail.getStates().getName();
			if (Utils.validateString(state)) {
				tv_state.setText(mAddressBookDetail.getStates().getName());
			}
		}
	...
	public MyAddress getAddressBookDetail() {
		...
		String statename = mAddressBookDetail.getStates().getName();
		if (null != statename) {
			addressBookDetail.setStateName(statename);
			String statecode = mAddressBookDetail.getStates().getCode();
			if (null != statecode) {
		...
		if (null != countryname) {
			...
			String countrycode = mAddressBookDetail.getCountry().getCode();
			if (null != countrycode) {
				addressBookDetail.setCountryCode(countrycode);
			}
		}
		if(null != mAddressBookDetail.getID()){
			addressBookDetail.setID(mAddressBookDetail.getID());
		}
	...
	public void setCountry(int type, String mCountry,
		...
		if (type == 0) {
			...
			mAddressBookDetail.setCountryCode(getCountryCode(mCountry,
					listCountry));
			mAddressBookDetail.getCountry().setName(mCountry);
			mAddressBookDetail.getCountry().setCode(getCountryCode(mCountry,
					listCountry));
		}
		if (type == 1) {
